...
namespace bcm3 {
	...
	bool SamplerPTChain::Initialize(size_t history_size, size_t history_subsampling)
	{
		...
		if (blocking_strategy->UsesClustering() && temperature != 0.0) {
			sample_history_clustering = std::make_shared<SampleHistoryClustering>(sampler->adapt_proposal_max_clustering_samples, sampler->history_clustering_nn, sampler->history_clustering_nn2, sampler->history_clustering_nclusters);
			current_cluster_assignment = -1;
		}
